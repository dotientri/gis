...
from parks.models import (
    ...
    CongVien, LoaiTienIch, TienIchCongVien, DanhMucSuCo, NguoiDung, HinhAnhCongVien
)
import json
import hashlib
...
class Command(BaseCommand):
    ...
    def create_parks_with_amenities(self):
        ...
        q2 = QuanHuyen.objects.get(ten_quan_huyen='Quận 2')
        q3 = QuanHuyen.objects.get(ten_quan_huyen='Quận 3')
        q10 = QuanHuyen.objects.get(ten_quan_huyen='Quận 10')
        q_bt = QuanHuyen.objects.get(ten_quan_huyen='Quận Bình Thạnh')
...
Công viên là nơi lý tưởng để các gia đình tươi vui, bạn bè gặp gỡ, và người lớn tuổi tập luyện thể dục sáng sớm. Đây cũng là điểm tham quan du lịch quan trọng tại TP.HCM, mô tả sự phát triển của thành phố qua các thập kỷ.''',
        ...
        self.add_park_images(park_tao_dan)
...
Với hệ thống tiện ích hoàn chỉnh và nhân viên chuyên nghiệp, Công viên Gia Định cam kết mang lại trải nghiệm tốt nhất cho mỗi du khách.''',
        ...
        self.add_park_images(park_gia_dinh)
...
Với quản lý bền vững, Công viên Bình Khánh cam kết bảo vệ hệ sinh thái và mang lại giá trị giáo dục cao cho du khách.''',
        ...
        self.add_park_images(park_binh_khanh)
        ...
        park_lvt, created = CongVien.objects.get_or_create(
            ma_code='LVT001',
            defaults={
                'ten_cong_vien': 'Công viên Lê Văn Tám',
                'ma_loai': park_type,
                'ma_trang_thai': status,
                'ma_quan_huyen': q1,
                'dia_chi': 'Võ Thị Sáu, Đa Kao, Quận 1',
                'dien_tich_m2': 60000,
                'dien_tich_cay_xanh': 40000,
                'toa_do_trung_tam': [10.7865, 106.6935],
                'don_vi_quan_ly': 'Sở Xây dựng TP.HCM',
                'mo_cua_24_7': True,
                'mien_phi_vao_cua': True,
                'nam_thanh_lap': 1985,
                'mo_ta': 'Công viên Lê Văn Tám là một không gian xanh mát nằm ngay giao lộ Võ Thị Sáu và Hai Bà Trưng. Nơi đây thường xuyên diễn ra các hội chợ sách và triển lãm hoa.',
                'da_xac_minh': True,
            }
        )
        if created:
            self.stdout.write(f'  ✓ Tạo: Công viên Lê Văn Tám')
        self.add_park_images(park_lvt)
        self.add_park_amenities(park_lvt, [
            ('nha_ve_sinh', 'Nhà vệ sinh', 'Nhà vệ sinh công cộng sạch sẽ.'),
            ('san_choi_tre', 'Sân chơi trẻ em', 'Khu vui chơi nhỏ cho trẻ em.'),
        ])

        # Công viên 23 tháng 9 (Quận 1)
        park_239, created = CongVien.objects.get_or_create(
            ma_code='239001',
            defaults={
                ...
                'ma_trang_thai': status,
                'ma_quan_huyen': q1,
                'dia_chi': 'Phạm Ngũ Lão, Phường Phạm Ngũ Lão, Quận 1',
                'dien_tich_m2': 90000,
                'dien_tich_cay_xanh': 50000,
                'toa_do_trung_tam': [10.7690, 106.6910],
                'don_vi_quan_ly': 'Trung tâm Quản lý Hạ tầng Kỹ thuật',
                'mo_cua_24_7': True,
                'mien_phi_vao_cua': True,
                'nam_thanh_lap': 1990,
                'mo_ta': 'Nằm dài theo đường Lê Lai và Phạm Ngũ Lão, công viên 23/9 là mảng xanh quan trọng của khu phố Tây Bùi Viện.',
                'da_xac_minh': True,
            }
        )
        if created:
            self.stdout.write(f'  ✓ Tạo: Công viên 23 tháng 9')
        self.add_park_images(park_239)
        self.add_park_amenities(park_239, [
            ('bai_do_xe', 'Bãi đỗ xe', 'Bãi giữ xe máy ngầm.'),
            ('san_the_thao', 'Sân thể thao', 'Khu vực tập dưỡng sinh.'),
        ])

        # Công viên Vinhomes Central Park (Bình Thạnh)
        park_vcp, created = CongVien.objects.get_or_create(
            ma_code='VCP001',
            defaults={
                'ten_cong_vien': 'Công viên Vinhomes Central Park',
                'ma_loai': park_type,
                'ma_trang_thai': status,
                'ma_quan_huyen': q_bt,
                'dia_chi': '208 Nguyễn Hữu Cảnh, Phường 22, Bình Thạnh',
                'dien_tich_m2': 140000,
                'dien_tich_cay_xanh': 80000,
                'dien_tich_mat_nuoc': 20000,
                'toa_do_trung_tam': [10.7950, 106.7220],
                'don_vi_quan_ly': 'Vinhomes',
                'mo_cua_24_7': True,
                'mien_phi_vao_cua': True,
                'nam_thanh_lap': 2016,
                'mo_ta': 'Công viên ven sông Sài Gòn lớn nhất TP.HCM với thiết kế cảnh quan tuyệt đẹp, lấy cảm hứng từ Central Park New York.',
                'da_xac_minh': True,
            }
        )
        if created:
            self.stdout.write(f'  ✓ Tạo: Công viên Vinhomes Central Park')
        self.add_park_images(park_vcp)
        self.add_park_amenities(park_vcp, [
            ('ho_nuoc', 'Hồ nước', 'Hồ cá Koi và đài phun nước.'),
            ('san_choi_tre', 'Sân chơi trẻ em', 'Nhiều khu vui chơi hiện đại.'),
            ('san_the_thao', 'Sân thể thao', 'Sân golf mini, sân tennis.'),
        ])

        # Công viên Lê Thị Riêng (Quận 10)
        park_ltr, created = CongVien.objects.get_or_create(
            ma_code='LTR001',
            defaults={
                'ten_cong_vien': 'Công viên Lê Thị Riêng',
                'ma_loai': park_type,
                'ma_trang_thai': status,
                'ma_quan_huyen': q10,
                'dia_chi': '875 Cách Mạng Tháng 8, Phường 15, Quận 10',
                'dien_tich_m2': 80000,
                'toa_do_trung_tam': [10.7870, 106.6660],
                'da_xac_minh': True,
            }
        )
        if created:
            self.stdout.write(f'  ✓ Tạo: Công viên Lê Thị Riêng')
        self.add_park_images(park_ltr)

    def add_park_images(self, park):
        """Thêm hình ảnh cho công viên"""
        # Danh sách ảnh mẫu chất lượng cao
        images_pool = [
            "https://images.unsplash.com/photo-1498084393753-b411b2d26b34?w=800&q=80", # Ghế đá công viên
            "https://images.unsplash.com/photo-1519331379826-f947873e307d?w=800&q=80", # Rừng cây
            "https://images.unsplash.com/photo-1585938389612-a552a28d6914?w=800&q=80", # Lối đi
            "https://images.unsplash.com/photo-1575995995938-7a6f2e5e7958?w=800&q=80", # Tượng đài/Cảnh quan
            "https://images.unsplash.com/photo-1562664377-709f2c337eb2?w=800&q=80", # Cây xanh
            "https://images.unsplash.com/photo-1596233064276-093322da7a00?w=800&q=80", # Hoa
            "https://images.unsplash.com/photo-1552664730-d307ca884978?w=800&q=80", # Hoạt động nhóm
            "https://images.unsplash.com/photo-1545569341-9eb8b30979d9?w=800&q=80", # Hoa anh đào/Cây cảnh
        ]
        
        # Xoay vòng ảnh dựa trên ID công viên để mỗi công viên có bộ ảnh hơi khác nhau
        start_idx = (park.ma_cong_vien * 2) % len(images_pool)
        
        # Xóa ảnh cũ nếu có để tránh trùng lặp
        HinhAnhCongVien.objects.filter(ma_cong_vien=park).delete()
        
        for i in range(5):
            idx = (start_idx + i) % len(images_pool)
            url = images_pool[idx]
            is_main = (i == 0)
            
            HinhAnhCongVien.objects.create(ma_cong_vien=park, url_anh=url, mo_ta=f'Cảnh quan {park.ten_cong_vien} - Ảnh {i+1}', la_anh_chinh=is_main, ngay_chup=timezone.now().date())
            
            if is_main:
                park.anh_dai_dien = url
                park.save()
